...
const InitializeDB: React.FC = () => {
  ...
  const handleAddSchedules = async () => {
    if (!window.confirm('🕐 This will add schedules to all existing courses. Continue?')) {
      return;
    }
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl p-8">
        ...
        <div className="space-y-3">
          ...
          <button
            ...
            disabled={loading}
            className="w-full bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white font-semibold py-4 px-6 rounded-lg transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg hover:shadow-xl transform hover:-translate-y-0.5"
          >
            {loading ? (
              <span className="flex items-center justify-center">
                <svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
        ...
        {scheduleResult && (
          <div className={`rounded-lg p-6 mb-6 mt-6 ${scheduleResult.success ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
            <div className="flex items-start space-x-3">
              ...
              <div className={scheduleResult.success ? 'text-green-800' : 'text-red-800'}>
                <p className="font-semibold mb-2">{scheduleResult.message}</p>
                {scheduleResult.success && scheduleResult.summary && (
                  <div className="text-sm space-y-1">
